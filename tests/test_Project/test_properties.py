...
def test_clone_repo(monkeypatch):
    with patch("gitpandas.repository.Repo.clone_from"):
        proj = ProjectDirectory(working_dir=['https://github.com/wdm0006/git-pandas.git'], verbose=True)
        assert "git-pandas" in proj.repo_name().values

        proj = ProjectDirectory(working_dir=['git://github.com/wdm0006/git-pandas.git'], verbose=True)
        assert "git-pandas" in proj.repo_name().values
#
#
